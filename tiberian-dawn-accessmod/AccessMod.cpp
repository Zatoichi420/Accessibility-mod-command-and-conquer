...
// Appends a line to %TEMP%\AccessMod_TiberianDawn.log so a "loaded but
// silent" report can be diagnosed without attaching a debugger. Best-effort:
// a logging failure here is never allowed to affect speech output.
void AccessMod_Log(const char* message)
{
	...
	strncat(path, "AccessMod_TiberianDawn.log", MAX_PATH - strlen(path) - 1);
...
const wchar_t* Vox_Text(int speech_index)
{
	...
	case VOX_NO_POWER:           return L"Insufficient power";
...
{
	Ensure_Tolk_Loaded();
	AccessMod_Log("AccessMod_Init reached: TiberianDawn.dll loaded and ran this code.");
	if (!Tolk_Output(L"Tiberian Dawn Accessibility Mod Loaded Successfully", false)) {
